...
const defaultOperationSettingInputs = {
  /* 额度相关 */
  QuotaForNewUser: 0,
  PreConsumedQuota: 0,
  QuotaForInviter: 0,
...
const OperationSetting = () => {
  let [inputs, setInputs] = useState(defaultOperationSettingInputs);
  ...
  const getOptions = async () => {
    ...
    if (success) {
      let newInputs = { ...defaultOperationSettingInputs };
      data.forEach((item) => {
        if (!(item.key in defaultOperationSettingInputs)) {
          return;
        }
        if (typeof defaultOperationSettingInputs[item.key] === 'boolean') {
